...
// @return {Object}
function getWrapperDefinition(url, options) {
	var resourceSlug,
		actionSlug,
		resourceClassName,
		apiMethodName,
	...
	if (!resourceClassName) {
		options.logger.silly('No matching api resource');
		next();
		return;
	}
		...
		function getDataFromApi(callback) {
			...
			if (resourceSlug === 'basket' && actionSlug !== 'create' &&
				...
				if (req.session.basketId) {
					...
					apiResource[apiMethodName](apiRequestParameters,
						boundCallback);
				} else {
				...
				apiResource[apiMethodName](apiRequestParameters, boundCallback);
			}
		}
